...
class WebAppView
{
    ...
    public function render(Request $request)
    {
		...
		$requestData = $request->getServerParams('actor');
		$requestData_keys = parse_url($requestData['REQUEST_URI']); // parse the url
		$url_segement = explode("/", $requestData_keys['path']); // splitting the path
		
		if($url_segement[2] == 'u'){
			//print_r($request->getQueryParams());
			$profile = User::where('username', $url_segement[3])->first();
			
			$view->title = ucfirst(($profile['attributes']['username'] ? $profile['attributes']['username'] : $url_segement[3])).' - '.$this->buildTitle(array_get($forum, 'data.attributes.title'));
			$view->description = ($profile['attributes']['bio'] ? $profile['attributes']['bio'] : $view->title).' - '.$this->buildDescription(array_get($forum, 'data.attributes.description'));					
			$view->image = $profile['attributes']['avatar_path'] ? array_get($forum, 'data.attributes.baseUrl').'/assets/avatars/'.$profile['attributes']['avatar_path'] : array_get($forum, 'data.attributes.logoUrl');
			$view->url = array_get($forum, 'data.attributes.baseUrl').'/u/'.$profile['attributes']['username'];
								
		}elseif($url_segement[2] == 't' || $url_segement[2] == 'd' || $url_segement[2] == 'p'){			
			$view->title = $this->title ? $this->title : ucfirst($url_segement[3]);
			$view->description = $view->title.' - '.$this->buildDescription(array_get($forum, 'data.attributes.description'));
			$view->image = array_get($forum, 'data.attributes.logoUrl');
			$view->url = array_get($forum, 'data.attributes.baseUrl').'/'.$url_segement[2].'/'.$url_segement[3];			
		}else{
			...
			$view->image = array_get($forum, 'data.attributes.logoUrl');
			$view->url = array_get($forum, 'data.attributes.baseUrl');
		}
    ...
    {
        return ($this->description ? $this->description.' - ' : '').$forumDescription;
    }
